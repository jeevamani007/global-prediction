...
class GovernmentDomainDetector:

    def __init__(self):
        self.domain = "Government"

        try:
            # Load keywords and columns from government_keywords table
            with engine.connect() as conn:
                result = conn.execute(text("SELECT keyword, column_name FROM government_keywords"))
    ...
    def value_pattern_score(self, series, column_name):
        ...
        # Numeric columns
        if any(k in name for k in ["amount", "paid", "income", "budget", "expenditure"]):
            if pd.to_numeric(col, errors="coerce").notna().mean() > 0.8:
                score += 1

        # ID columns
        if any(k in name for k in ["id", "number", "passport"]):
    ...
    def predict(self, csv_path):
        ...
        for col in df.columns:
            ...
            # Fuzzy match with keywords and columns from DB
        ...
        if confidence_100 >= 85:
            decision = "CONFIRMED_GOVERNMENT"
            qualitative = "Very Strong"
        elif confidence_100 >= 65:
            decision = "LIKELY_GOVERNMENT"
            qualitative = "Strong"
        else:
            decision = "UNKNOWN"
            qualitative = "Weak"
