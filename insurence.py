...
class InsuranceDomainDetector:

    def __init__(self):
        self.domain = "Insurance"

        try:
            # Read keywords from insurance_keywords table
            with engine.connect() as conn:
                result = conn.execute(text("SELECT keyword, column_name FROM insurance_keywords"))
        ...
        # Optional synonyms for insurance
        self.synonyms = {
            "pol": "policy",
            "prem": "premium",
            "cust": "customer",
    ...
    def value_pattern_score(self, series, column_name):
        ...
        # Numeric columns
        if any(k in name for k in ["amount", "premium", "sum", "claim"]):
    ...
    def predict(self, csv_path):
        ...
        if confidence_100 >= 85:
            decision = "CONFIRMED_INSURANCE"
            qualitative = "Very Strong"
        elif confidence_100 >= 65:
            decision = "LIKELY_INSURANCE"
            qualitative = "Strong"
        else:
            decision = "UNKNOWN"
            qualitative = "Weak"
