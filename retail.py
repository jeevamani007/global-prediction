...
class RetailDomainDetector:

    def __init__(self):
        self.domain = "Retail"

        try:
            # Read from retail_keywords table
            with engine.connect() as conn:
                result = conn.execute(text("SELECT keyword, column_name FROM retail_keywords"))
        ...
        # Optional synonym mapping for retail
        self.synonyms = {
            "prod": "product",
            "qty": "quantity",
            "amt": "amount",
    ...
    def value_pattern_score(self, series, column_name):
        ...
        if any(k in name for k in ["amount", "amt", "price", "total"]):
    ...
    def predict(self, csv_path):
        ...
        if confidence_100 >= 85:
            decision = "CONFIRMED_RETAIL"
            qualitative = "Very Strong"
        elif confidence_100 >= 65:
            decision = "LIKELY_RETAIL"
            qualitative = "Strong"
        else:
            decision = "UNKNOWN"
            qualitative = "Weak"
