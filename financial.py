...
class FinancialDomainDetector:
    ...
    def __init__(self):
        ...
        try:
            # Read keywords & column names from financial_keywords table
            with engine.connect() as conn:
                result = conn.execute(text("SELECT keyword, column_name FROM financial_keywords"))
            ...
            print(f"Warning: Could not load financial keywords: {e}")
            self.keywords = []
            self.columns_db = []

        # Synonyms mapping (can be same as banking)
        self.synonyms = {
            "acct": "account",
            "accno": "account",
            "cust": "customer",
    ...
    def predict(self, csv_path):
        ...
        for col in df.columns:
            ...
            # 1️⃣ Keyword + column fuzzy match
            ...
            # Store matched keywords & mapping
            if best_score >= 75:
        ...
        if confidence_100 >= 85:
            decision = "CONFIRMED_FINANCIAL"
            qualitative = "Very Strong"
        elif confidence_100 >= 65:
            decision = "LIKELY_FINANCIAL"
            qualitative = "Strong"
        else:
            decision = "UNKNOWN"
            qualitative = "Weak"
