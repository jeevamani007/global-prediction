...
class HealthcareDomainDetector:
    def __init__(self):
        self.domain = "Healthcare"

        try:
            # Load keywords and columns from healthcare_keywords table
            with engine.connect() as conn:
                result = conn.execute(text("SELECT keyword, column_name FROM healthcare_keywords"))
        ...
        # Optional synonyms for healthcare
        self.synonyms = {
            "pat": "patient",
            "doc": "doctor",
            "med": "medicine",
    ...
    def value_pattern_score(self, series, column_name):
        ...
        if any(k in name for k in ["amount", "cost", "price"]):
            if pd.to_numeric(col, errors="coerce").notna().mean() > 0.8:
                score += 1

        if "id" in name or "patient" in name or "doctor" in name:
    ...
    def predict(self, csv_path):
        ...
        for col in df.columns:
            ...
            # 1️⃣ fuzzy match with keywords and columns
        ...
        if confidence_100 >= 85:
            decision = "CONFIRMED_HEALTHCARE"
            qualitative = "Very Strong"
        elif confidence_100 >= 65:
            decision = "LIKELY_HEALTHCARE"
            qualitative = "Strong"
        else:
            decision = "UNKNOWN"
            qualitative = "Weak"
