                    ...
                    for file_path in file_paths:
                        try:
                            ...
                            if dynamic_rules and dynamic_rules.get('columns'):
                                ...
                                all_dynamic_rules[file_key] = dynamic_rules
    ...
    try:
        ...
        for file in files:
            ...
            if '/' in file.filename:
                ...
                # parts[0] is the uploaded Root folder
                
                if len(parts) >= 3:
                    # format: Root/App1/file.csv
                    # format: Root/App1/Data/file.csv
                    # We want to group by "App1" (parts[1])
                    folder_name = parts[1]
                elif len(parts) == 2:
                    # format: Root/file.csv
                    # Files directly in root → Group as "Root" (or use Root folder name)
                    folder_name = parts[0]
                else:
                    folder_name = "default_folder"
                    
                actual_filename = parts[-1]
                print(f"DEBUG:   -> Assigned to App: {folder_name}")
            else:
                # Default: all files in one folder
                folder_name = "default_folder"
                actual_filename = file.filename
                print(f"DEBUG:   -> No folder, assigned to default")
        ...
        for folder_name, file_paths in folder_files.items():
            try:
                ...
                # Convert all numeric types to native Python for JSON serialization
                import numpy as np
                def convert_to_native(obj):
                    """Convert numpy/pandas types to native Python types for JSON serialization"""
                    # Use numpy.integer and numpy.floating for NumPy 2.0 compatibility
                ...
                app_type_result = convert_to_native(app_type_result)
                print(f"📊 Application Type Detection for {folder_name}:")
                print(f"   Type: {app_type_result.get('application_type', 'Unknown')}")
                print(f"   Confidence: {app_type_result.get('confidence', 0)}%")
                print(f"   Patterns: {app_type_result.get('patterns_detected', [])}")
                ...
                for filename in folder_analysis['csv_files']:
                    ...
                    try:
                        ...
                        if dynamic_rules and dynamic_rules.get('columns'):
                            ...
                            print(f"✅ Generated {len(summary.get('business_rules', []))} business rules for {filename}")
                            print(f"   Total columns: {summary.get('summary', {}).get('total_columns_analyzed', 0)}")
                            print(f"   Critical: {len([c for c in dynamic_rules.get('critical_columns', [])])}")
                            print(f"   Warning: {len([c for c in dynamic_rules.get('warning_columns', [])])}")
                            print(f"   Safe: {len([c for c in dynamic_rules.get('safe_columns', [])])}")
                    except Exception as e_rules:
                        import traceback
                        traceback.print_exc()
                        print(f"Warning: Could not generate business rules for {filename}: {str(e_rules)}")
                        business_rules_per_file[filename] = {'error': str(e_rules)}
                ...
                try:
                    ...
                    application_purpose = purpose_analyzer.analyze_from_rules_data(
                        combined_rules,
                        relationships=detailed_relationships if isinstance(detailed_relationships, list) else detailed_relationships.get('relationships', [])
                    )
                    
                    # Ensure explanation_points exists for frontend
                    if application_purpose and 'explanation_points' not in application_purpose:
                        explanation_points = []
                        if application_purpose.get('line1'):
                            explanation_points.append(application_purpose['line1'])
                        if application_purpose.get('line2'):
                            explanation_points.append(application_purpose['line2'])
                        application_purpose['explanation_points'] = explanation_points
                        print(f"✅ Added explanation_points array for {folder_name}")
                    
                    print(f"✅ Generated application purpose for {folder_name}:")
                    print(f"   Line1: {application_purpose.get('line1', '')[:80]}...")
                    print(f"   Line2: {application_purpose.get('line2', '')[:80]}...")
                    print(f"   Points: {len(application_purpose.get('explanation_points', []))}")
                except Exception as e:
                ...
                folders_results[folder_name] = {
                    ...
                    'file_relationships': detailed_relationships if isinstance(detailed_relationships, list) else detailed_relationships.get('relationships', []),
        ...
        # Build response
        # Convert all results to native Python types for JSON serialization
        folders_results = convert_to_native(folders_results)
        
        # Validate data structure before returning
        print(f"\n🔍 Validating folder results before sending to frontend...")
        for folder_name, folder_data in folders_results.items():
            if 'error' not in folder_data:
                # Validate application_type
                app_type = folder_data.get('application_type', {})
                if not app_type or not isinstance(app_type, dict):
                    print(f"⚠️ WARNING: {folder_name} missing application_type structure")
                elif not app_type.get('confidence'):
                    print(f"⚠️ WARNING: {folder_name} has 0 or missing confidence: {app_type.get('confidence')}")
                else:
                    print(f"✅ {folder_name}: App type = {app_type.get('application_type', 'Unknown')}, Confidence = {app_type.get('confidence')}%")
                
                # Validate business_rules_summaries
                business_summaries = folder_data.get('business_rules_summaries', {})
                if not business_summaries or not isinstance(business_summaries, dict):
                    print(f"⚠️ WARNING: {folder_name} missing business_rules_summaries")
                else:
                    print(f"✅ {folder_name}: Business rules for {len(business_summaries)} files")
                
                # Validate application_purpose
                app_purpose = folder_data.get('application_purpose')
                if not app_purpose:
                    print(f"⚠️ WARNING: {folder_name} missing application_purpose")
                elif not app_purpose.get('explanation_points'):
                    print(f"⚠️ WARNING: {folder_name} application_purpose missing explanation_points array")
                    print(f"   Available keys: {list(app_purpose.keys())}")
                else:
                    print(f"✅ {folder_name}: Application purpose with {len(app_purpose.get('explanation_points', []))} points")
            else:
                print(f"❌ {folder_name}: Has error - {folder_data.get('error', 'Unknown')}")
        
        # Log final stats
        print(f"\n📦 Folder Upload Summary:")
        print(f"   Total folders processed: {len(folders_results)}")
        for folder_name, folder_data in folders_results.items():
            if 'error' not in folder_data:
                print(f"   ✅ {folder_name}:")
                print(f"      - Application Type: {folder_data.get('application_type', {}).get('application_type', 'Unknown')}")
                print(f"      - Confidence: {folder_data.get('application_type', {}).get('confidence', 0)}%")
                print(f"      - CSV Files: {len(folder_data.get('csv_files', []))}")
                print(f"      - Relationships: {len(folder_data.get('file_relationships', []))}")
                print(f"      - Business Rules Files: {len(folder_data.get('business_rules_summaries', {}))}")
            else:
                print(f"   ❌ {folder_name}: {folder_data.get('error', 'Unknown error')}")
...
@app.get("/multi-analysis", response_class=HTMLResponse)
async def multi_application_page(request: Request):
    """Return the multi_application.html page for multi-folder analysis results"""
    return templates.TemplateResponse("multi_application.html", {"request": request})
